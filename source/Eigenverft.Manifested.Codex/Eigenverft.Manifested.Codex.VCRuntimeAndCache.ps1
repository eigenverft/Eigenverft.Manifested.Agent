<#
    Eigenverft.Manifested.Codex.VCRuntimeAndCache
#>
...
function Get-VCRuntimeInstallerInfo {
    ...
    [pscustomobject]@{
        ...
        CachePath    = (Join-Path $mgr.VCRuntimeCacheRoot 'vc_redist.x64.exe')
    }
}
...
function Ensure-VCRuntimeInstaller {
    [CmdletBinding()]
    param(
    ...
    $mgr = Get-CodexManagerLayout -LocalRoot $LocalRoot
    $info = Get-VCRuntimeInstallerInfo -LocalRoot $LocalRoot
    Ensure-CodexDirectory -Path $mgr.VCRuntimeCacheRoot
    ...
    $cached = Get-CachedVCRuntimeInstaller -LocalRoot $LocalRoot
...
function Invoke-VCRuntimeInstaller {
    ...
    $logPath = Join-Path $mgr.VCRuntimeCacheRoot ("vc_redist.install.$timestamp.log")
...
function Ensure-VCRuntime {
    ...
    $installer = Ensure-VCRuntimeInstaller -RefreshVCRuntime:$RefreshVCRuntime -LocalRoot $LocalRoot
    ...
    Write-Host 'Installing Microsoft Visual C++ Redistributable prerequisites for Codex sandboxing...'
    $installResult = Invoke-VCRuntimeInstaller -InstallerPath $installer.Path -TimeoutSec $InstallTimeoutSec -LocalRoot $LocalRoot
